...
contract Octalily is IERC20, MultiOwned, IOctalily {
    ...
    function buy(uint256 amount) public override {
        address superSmartInvestor = msg.sender;
        pairedToken.transferFrom(superSmartInvestor, address(this), amount);
        _mint(superSmartInvestor, amount / price);
    }

    function sell(uint256 amount) public override {
        address notGunnaMakeIt = msg.sender;
        require (balanceOf(notGunnaMakeIt) >= amount);
        _burn(notGunnaMakeIt, amount);
        uint256 exitAmount = (amount - amount * totalFees / 10000) * price;
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        emit Transfer(address(0), address(this), reflection);
        emit Transfer(address(0), feeCollection, fees);
    }

    function _burn(address account, uint amount) internal {
        uint256 fees = amount * 123 / 10000;
        uint256 reflection = amount * rootflectionFeeRate / 10000;
        uint256 remaining = amount.sub(fees + reflection);
        ...
        emit Transfer(account, address(0), remaining);
        emit Transfer(account, address(this), reflection);
        emit Transfer(account, feeCollection, fees);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 remaining = amount.sub(rootflectionFee + burn + fee);
